...
impl AlpmHelper {
    pub fn new() -> Self {
        Self {
            pkg_list_install: Vec::new(),
            pkg_list_removal: Vec::new(),
        }
    }
    ...
    fn install_apps(&self, pkg_list: &Vec<String>, install: bool) -> bool {
        ...
        if Path::new("/sbin/pamac-installer").exists() {
            ...
            Exec::shell(format!("pamac-installer {arg} {packages_do}"))
                .join()
                .unwrap();
        } else {
